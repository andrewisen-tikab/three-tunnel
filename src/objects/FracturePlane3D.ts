	...
	implements AbstractFracturePlane3D, AbstractObject3D
{
	...
	/** Internal plane mesh reference for intersection tests */
	...
	private _build(): void {
		...
		this._mesh = plane;
		// Build BVH tree for intersection queries if not present
		if (
			!("boundsTree" in plane.geometry) &&
			"computeBoundsTree" in plane.geometry
		) {
			(
				plane.geometry as THREE.BufferGeometry & {
	...
	/** Access underlying mesh (may be null before build) */
