 ...
 */
function evenlyInterpolateShape(
	shape: THREE.Shape,
	desiredPointSpacing: number = 0.1,
) {
	...
	for (let i = 1; i < shapePoints.length; i++) {
		...
		const numSubdivisions = Math.ceil(
			(segmentLength / totalLength) * numPointsToAdd,
		);
...
export default class Tunnel3D extends THREE.Object3D implements AbstractTunnel3D
{
	...
	private _build() {
		...
		const tunnelGeometry = new THREE.ExtrudeGeometry(
			tunnelShape,
			extrudeSettings,
		);
			...
			this._tunnelGroup.traverse((obj) => {
				...
				const apply = (mat: THREE.Material) => {
					if ("clippingPlanes" in mat) {
						(mat as THREE.MeshBasicMaterial).clippingPlanes =
	...
	public mockDoStick(
		...
		const newMockStickPosition = this.mockGetStick(
			point,
			stickDistance,
			tolerance,
		);
		...
		const adjustedIndex = Math.round(
			(newMockStickPosition.index / max2) * max1,
		);
		...
		grout.lookAt(
			newPointAtTunnelShape.point.x,
			newPointAtTunnelShape.point.y,
			initialZ,
		);
	}
	...
	public mockGetStick(
		...
		while (conditionMet === false) {
			...
			const distance = startingPointAtShape.point.distanceTo(
				newPointAtShapeToCompare.point,
			);
	...
	public mockGetPointAtShape(
		...
		const startPoint =
			(startingPoint as Partial<PointAlongShape>)?.point ??
			(startingPoint as THREE.Vector2);
